...
| 2026-03-28 | Claude | ⚠️ **VOICE TRIGGER DETECTION PENDING** — Manual (voice-triggered) mode connects and captures mic successfully. VAD fires `turnComplete` but `FIND_VIDEOS:` signal not yet confirmed end-to-end. TEXT-only modality is configured; may need further debugging of system instruction or signal format. Deferred by user. |
