...
| 2026-03-28 | Claude | ✅ **BACKEND MODEL FIX** — `backend/app/services/gemini.py`: `model_analyze_fast` and `model_solve_fast` were pointing to `"gemini-3.1-flash-preview"` (non-existent model ID) → changed to `"gemini-2.5-flash"`. Also added LaTeX backslash fallback in `_parse_json()`: on `JSONDecodeError`, escapes raw `\frac`, `\sin` etc. before retrying `json.loads`. Fixes `/solve` 500 on math problems. |
| 2026-03-28 | Claude | ✅ **VOICE-TRIGGERED MODE** — Chrome extension now supports two detection modes. `DetectionModeSetting` component added to Settings page ("Screen Recording" section). Settings sync via `window.postMessage` → content script → `chrome.storage.local`. `chrome.storage.onChanged` listener in content.js keeps `detectionMode` in sync across ALL tabs. Manual mode: TEXT-only response modality + mic capture (getUserMedia → ScriptProcessor → PCM 16kHz → WS). Auto mode unchanged (AUDIO + transcription). Both modes use separate system instructions. |
| 2026-03-28 | Claude | ✅ **EXTENSION TOGGLE FIX** — Switching detection mode while recording was active left the mic stream alive and broke the next auto session. Fixed: `chrome.storage.onChanged` handler now calls `stopMonitoring()` if recording is in progress when mode changes. `ws.onclose` now always calls `stopMicCapture()` + `stopPlayback()` so resources are released even on unexpected 1011 close. Auto/manual toggle confirmed working. |
| 2026-03-28 | Claude | ✅ **VOICE-TRIGGERED MODE FULLY WORKING** — End-to-end voice conversation + video search confirmed working. Full fix sequence: (1) Switched manual mode from TEXT→AUDIO modality + function calling (`find_videos` tool declaration) to fix 1011; (2) Added green 🎤 push-to-talk button above main button — tapping sends `realtimeInput.activityEnd` after capturing a fresh frame, so model sees exact screen state at request time; (3) Disabled VAD via `realtimeInputConfig.automaticActivityDetection.disabled:true` for reliable push-to-talk; (4) Fixed audio mimeType check to `startsWith("audio/pcm")` with dynamic sample rate extraction; (5) Added `generationConfig.speechConfig.languageCode:"en-US"` to force English voice output. `find_videos` function call arrives as top-level `msg.toolCall` (not inside `serverContent`). |
| 2026-03-28 | Claude | ✅ **EXTENSION LIVE MODE COMPLETE** — Chrome extension voice/live mode fully implemented and confirmed working end-to-end. Two detection modes: Auto (screen monitoring, AUDIO modality + transcription) and Manual/Voice (push-to-talk 🎤 button, mic capture → PCM 16kHz → Gemini Live API, `find_videos` function calling). Settings sync via `window.postMessage` → content.js → `chrome.storage.local`. Echo cancellation enabled on mic. Verbal concept search triggers full StudySnap pipeline (localStorage frame → `/processing`). |
